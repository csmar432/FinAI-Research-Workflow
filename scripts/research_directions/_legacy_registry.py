"""Legacy inline registry — 备份 2026-06-29.

这是 DirectionFactory._init_registry() 的原始内联实现（45 个方向的
硬编码 dataclass 字面量，~1,972 行）。迁移到 directions.yaml 后作为参考保留，
不在生产代码中调用。

如需恢复内联模式：
    from scripts.research_directions._legacy_registry import _init_registry_legacy
    # 然后把 DirectionFactory._init_registry 替换为 _init_registry_legacy

P0-6 修复第二阶段 (2026-06-29): 删除内联 dataclass，改为 _load_from_yaml。
等价性验证：45 个方向 keys 1:1 一致，关键字段 (direction_name/display_name/
literature_theme/methodology_chain.steps) 全部相同。
"""

from __future__ import annotations

# 此文件包含原 _init_registry 方法的源码（备份参考），不直接导入使用。
# 原始代码片段（作为字符串保留）：
_LEGACY_INIT_REGISTRY_SOURCE = """        # ── Green Finance ──────────────────────────────────────────────────
