...
% ─────────────────────────────────────────────────────────────────────────────
% Font setup — Times + Helvetica (no CJK fonts; all text is English)
% ─────────────────────────────────────────────────────────────────────────────
...
\begin{enumerate}
    ...
    \item \textbf{30 journal templates} (JF, JFE, RFS, Journal of Financial Economics; Jingji Yanjiu (Economic Research Journal), Jinrong Yanjiu (Journal of Financial Research), Guanli Shijie (Management World); JPE, Econometrica; and 20 others).
...
Existing tools for automated econometrics include \texttt{estimagic}, \texttt{causalinference}, and \texttt{diff-in-diff2}.  These packages automate specific estimation tasks but do not integrate with the broader research workflow (literature search, writing, formatting).
...
All data are sourced from \texttt{~/Desktop/empirical-data/} and staged with
standardized merge keys (6-digit firm code + year).%
\input{../../output/fin-experiments/case_studies/results/table_cs1_lcc.tex}
...
(Size, Lev, ROA, Growth, Top1, ListAge) are from the
Listed-Firm Control Variables Dataset (B13, 2006--2024, winsorized at 1\%/99\%).
All datasets are Chinese administrative records translated to English.
...
\input{../../output/fin-experiments/case_studies/results/table_cs2_gc.tex}
...
stock code and year.  The outcome is the SA index, where higher values indicate greater
...
\input{../../output/fin-experiments/case_studies/results/table_cs3_bd.tex}
...
% ─────────────────────────────────────────────────────────────────────────────
% REFERENCES — manually formatted (tectonic has no .bst support)
% ─────────────────────────────────────────────────────────────────────────────
\newpage
\section*{References}

\noindent\hangindent=0.6cm\hangafter=1
GPT-4 Technical Report.  2023.  arXiv:2303.08774.  OpenAI.  \url{https://arxiv.org/abs/2303.08774}.

\noindent\hangindent=0.6cm\hangafter=1
Wang, L.  2019.  arXiv-sanity-lm: Language models for scientific paper understanding.  arXiv preprint.

\noindent\hangindent=0.6cm\hangafter=1
Anthropic.  2024.  Claude 3.5 Sonnet.  \url{https://www.anthropic.com/news/claude-3-5-sonnet}.

% For full bibliography from references.bib, compile with:
%   pdflatex finai && bibtex finai && pdflatex finai && pdflatex finai
% (BibTeX requires a .bst file not bundled with tectonic)
