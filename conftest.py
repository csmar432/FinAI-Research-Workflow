...
implicitly avoided sys.path issues by using explicit test lists per
job. Unifying into 'pytest tests/' exposes that the only sys.path
injection came from tests/conftest.py, which pytest-xdist worker
subprocesses may run after some module imports have already been
attempted.
