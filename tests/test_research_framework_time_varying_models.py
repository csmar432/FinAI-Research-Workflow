"""tests/test_research_framework_time_varying_models.py — Deep tests for time_varying_models."""
...
class TestExports:
    def test_has_time_varying_class(self):
        for n in dir(mod):
            if not n.startswith("_") and isinstance(getattr(mod, n, None), type):
                if any(s in n.lower() for s in ["varying", "tvp", "kalman"]):
                    return
        # OK if no class
